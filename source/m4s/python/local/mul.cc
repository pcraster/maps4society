...
calc::Field* mul(
  ...
  if(field_b->isSpatial() == false){
    ...
    return mul_field_number(epol, &arg1, nonspatial_value(field_b), &res);
  }
  else if(field_a->isSpatial() == false){
    const multicore_field::multicore_field<REAL4> arg2(field_b);
    return mul_number_field(epol, nonspatial_value(field_a), &arg2, &res);
  }
  else{
    const multicore_field::multicore_field<REAL4> arg1(field_a);
    const multicore_field::multicore_field<REAL4> arg2(field_b);
    return mul_field_field(epol, &arg1, &arg2, &res);
  }
}
